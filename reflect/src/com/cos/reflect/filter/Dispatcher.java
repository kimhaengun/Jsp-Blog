...
import com.cos.reflect.RequestMapping;
...
//		}
		...
		//그 파일에 메서드만 호출  / tostring,get,set함수 탐색
...
//			}
//		}
		for (Method method : methods) { //4번 실행(join, login, user, hello)
...
//			System.out.println(requestMapping.value());
			if(requestMapping.value().equals(endPoint)) {
				isMatching = true;
				try {
//					String path = (String)method.invoke(userController);//invoke=메소드를 실행

					Parameter[] params = method.getParameters(); //login일 경우 login(LoginDto dto)
					String path = null;
//					if(params.length != 0) {
...
//						// username, password값을 가져와 set함수에 넣어주기 위해 
...
//					}
					if(params.length!=0) {
						//해당 dtoInstance를 리플렉션해서 set함수를 호출한다(usernaeme,password)
						Object dtoInstance = params[0].getType().newInstance();
//						params[0].getType().getDeclaredConstructor().newInstance();
